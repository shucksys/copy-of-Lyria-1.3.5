(function() {
  ...
  var url             = 'http://game.granbluefantasy.jp/';
  var jQueryCache     = {};
  var isJST           = true;
  var times           = {};
  var timeZone        = '';
  ...
  $(document).on("keypress", function (e) {
    switch (e.key) {
      ...
      case '8':
      case '9':
        if (connectedTabIDs[e.key - 1] !== undefined) {
          if ($('#window-' + e.key).length > 0) {
            $('.window-btn').removeClass('selected');
            $('#window-' + e.key).addClass('selected');
            selectedTabID = parseInt(connectedTabIDs[e.key - 1]);
          }
        }
        break;
      default:
  ...
  var processUIUpdate = function (message) {
    ...
    switch (type) {
      ...
      case 'setTimeZone':
        ...
        break;
      case 'addItem':
        addItem(data.id, data.category, data.number, data.name, data.sequence, data.tooltip);
        break;
      case 'hideObject':
      ...
      case 'autoRepeat':
        if (toggleRepeat) {
          ...
          if (jQueryCache['#quest-repeat'].data('url') !== undefined && jQueryCache['#quest-repeat'].data('url') !== '') {
            openURL(url + jQueryCache['#quest-repeat'].data('url'));
          }
        }
        break;
      case 'updateConnections':
        connectedTabIDs = data;
        $('.window-btn').each(function (index) {
          if (connectedTabIDs[index] !== undefined) {
            $(this).show();
          } else {
            $(this).hide();
          }
